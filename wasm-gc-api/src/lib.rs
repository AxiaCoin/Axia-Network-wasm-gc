...
impl GcResult {
    ...
    /// `axia_wasm::Module`.
    ///
    /// If your crate's `axia_wasm` crate is a different version than this
...
fn _gc_file(input: &Path, output: &Path) -> Result<(), Error> {
    let mut module = axia_wasm::deserialize_file(input)
        .map_err(error::from)?
        .parse_names()
        .map_err(|(mut l, _)| l.remove(0).1)
        .map_err(error::from)?;
    Config::new()._gc(&mut module);
    axia_wasm::serialize_to_file(output, module).map_err(error::from)?;
